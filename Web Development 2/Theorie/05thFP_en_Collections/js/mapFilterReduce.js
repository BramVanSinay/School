...
const vb = animals.filter(({name})=>value.name.startsWith("c"));
console.log(vb);
...
const newAnimals = animals.map(({name,size:oldSize,weight})=>{
  return ({name,size : oldSize ==="small"?"S": oldSize === "medium"?"M" : "L",weight})
});
...
const numberOfSmallAnimals = animals.reduce((pv,{size})=>(size==="small" ? ++pv:pv),0);
