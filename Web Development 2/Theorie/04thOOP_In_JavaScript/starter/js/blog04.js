...
class Blog {
  ...
  get naamCreator(){
    return this.#naamCreator
  }
  set naamCreator(naamCreator){
    this.#naamCreator = naamCreator;
  }
  get nrOfEntries(){
  ...
  get freeSpace(){
    return (Math.round((Blog.maxEntries - this.nrOfEntries) / blog.maxEntries*100));
  }

  contains(searchText){
    for (const element of this.#entries){
      if(element.contains(searchText)) return true;
    }
  }
  addEntry(body){
